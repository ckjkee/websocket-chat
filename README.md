...
### 3. Тестирование
Откройте `test_session.html` для тестирования API endpoints.
...
### Проблема: Пользователь не выкидывается автоматически
**Решение**: Проверьте консоль браузера на ошибки JavaScript, убедитесь что `/session-info` endpoint работает.

### Проблема: Таймер не обновляется
**Решение**: Проверьте что `timerUpdateInterval` создается в `setupSessionTimers`.

### Проблема: Сессия истекает раньше времени
**Решение**: Проверьте настройки времени в `GenerateToken` и синхронизацию времени между клиентом и сервером.
